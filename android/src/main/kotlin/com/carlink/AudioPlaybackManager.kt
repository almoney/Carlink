...
/**
 * AudioPlaybackManager - Native AudioTrack-based audio playback for CPC200-CCPA audio streams
 *
 * PURPOSE:
 ...
 *
 * AUDIO FORMAT SUPPORT (CPC200-CCPA Protocol):
 * - Format 1-2: 44100Hz stereo 16-bit (music)
 * - Format 3: 8000Hz mono 16-bit (phone calls)
 * - Format 4: 48000Hz stereo 16-bit (high-quality audio)
 * - Format 5: 16000Hz mono 16-bit (Siri/voice assistant)
 * - Format 6: 24000Hz mono 16-bit (enhanced voice)
 * - Format 7: 16000Hz stereo 16-bit (stereo voice)
 *
 * THREAD SAFETY:
 * All AudioTrack operations are synchronized to prevent concurrent access issues.
 * Write operations happen on the caller's thread (typically USB I/O thread).
 *
 * PERFORMANCE:
 * Uses AudioTrack.MODE_STREAM for continuous playback with minimal latency.
 * Buffer size is calculated based on minimum requirements plus safety margin.
 */
    ...
    fun initialize(format: AudioFormatConfig = AudioFormats.FORMAT_4): Boolean {
        synchronized(lock) {
            try {
                ...
                log("[AUDIO] Initialized: ${format.sampleRate}Hz ${format.channelCount}ch buffer=${actualBufferSize}B (min=${minBufferSize}B)")
                return true
    ...
    fun getStats(): Map<String, Any> {
        synchronized(lock) {
            ...
            return mapOf(
                ...
                "currentFormat" to (currentFormat?.let {
