...
class AudioRingBuffer(
     ...
     */
    fun read(out: ByteArray, offset: Int = 0, length: Int = out.size - offset): Int {
     ...
     */
    fun fillLevel(): Float {
        return availableForRead().toFloat() / capacity
    }
     ...
     */
    fun fillLevelMs(): Int {
        return if (bytesPerMs > 0) availableForRead() / bytesPerMs else 0
    }
     ...
     */
    fun hasEnoughData(thresholdMs: Int = 50): Boolean {
        return fillLevelMs() >= thresholdMs
    }
    ...
    companion object {
         ...
         */
        fun forMedia(sampleRate: Int = 44100, channels: Int = 2): AudioRingBuffer {
            return AudioRingBuffer(
                capacityMs = 250,  // 250ms buffer for media
         ...
         */
        fun forNavigation(sampleRate: Int = 44100, channels: Int = 2): AudioRingBuffer {
            return AudioRingBuffer(
                capacityMs = 120,  // 120ms buffer for navigation
                sampleRate = sampleRate,
                channels = channels,
            )
        }
