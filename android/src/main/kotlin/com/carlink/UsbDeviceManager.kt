...
/**
 * UsbDeviceManager - Centralized USB Device Management for Carlink
 *
 * PURPOSE:
 ...
 *
 * RESPONSIBILITIES:
 * - Device Discovery: Scanning, filtering, and identification of Carlinkit hardware
 * - Permission Management: Requesting and tracking USB permissions with Android 13+ compliance
 * - Connection Lifecycle: Opening, closing, and resetting USB device connections
 * - Configuration Management: Setting configurations and claiming/releasing interfaces
 * - VID/PID Filtering: Known Carlinkit VID/PID combinations (0x1314/0x08e4)
 *
 * THREAD SAFETY:
 * This class is designed to be called from the main thread for permission callbacks.
 * Device operations are thread-safe and can be called from any thread.
 *
 * ANDROID COMPATIBILITY:
 * - minSdk 32 (Android 12)
 * - Handles Android 13+ (TIRAMISU) receiver registration requirements
 * - Uses PendingIntent.FLAG_MUTABLE for permission requests
 *
        ...
        if (requestPermission && !hasPermission) {
            ...
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    ...
                    // For API 32 and below, use standard registration
    ...
    fun requestPermission(
        ...
        if (usbManager.hasPermission(device)) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                // For API 32, use standard registration
    ...
    private fun cleanupPermissionReceiver() {
        ...
        permissionReceiver?.let { receiver ->
            try {
                ...
                // Receiver was already unregistered - this is expected
                log("Permission receiver already unregistered")
            }
